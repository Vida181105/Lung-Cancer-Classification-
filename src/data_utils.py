...
Carried over from the v2 attempt essentially unchanged: the dataset handling
was never the broken part, and re-deriving a working resolver/audit/split would
have burned CPU-budget time for nothing. See ``archive/src_v2/data_utils.py``
for the original.
...
* The dataset ships with duplicate files that straddle the train/test boundary
  (LESSON 6). :func:`audit_leakage` quantifies this and
...
# --------------------------------------------------------------------------
# Duplicate / leakage audit (LESSON 6)
# --------------------------------------------------------------------------
...
def make_split_datasets(split_df: pd.DataFrame, img_size=IMG_SIZE,
    ...
    ``one_hot=True`` is required whenever the model is compiled with label
    smoothing (LESSON 2), because ``CategoricalCrossentropy(label_smoothing=...)``
    has no sparse counterpart. ``train_utils.compile_model()`` reports which
    target format it expects, and ``evaluate_utils.predict()`` accepts either.
    """
    ...
    train_ds = make_dataset(frames["train"], img_size, batch_size,
                            ...
                            one_hot=one_hot)
    val_ds = make_dataset(frames["val"], img_size, batch_size, one_hot=one_hot)
    test_ds = make_dataset(frames["test"], img_size, batch_size, one_hot=one_hot)
